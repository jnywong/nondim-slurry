...
folder, _ = os.path.split(slurpy.__file__)

def premgravity(radius):
    # Read PREM data from CSV files
    prem_radius= genfromtxt(folder+'/lookupdata/radPREM.csv', delimiter=',')
    prem_gravity=genfromtxt(folder+'/lookupdata/gravPREM.csv', delimiter=',')
...
def premdensity(radius):
    # Read PREM data from CSV files
    prem_radius= genfromtxt(folder+'/lookupdata/radPREM.csv', delimiter=',')
    prem_density=genfromtxt(folder+'/lookupdata/denPREM.csv', delimiter=',')
...
def premvp(radius):
    # Read PREM data from CSV files
    prem_radius= genfromtxt(folder+'/lookupdata/radPREM.csv', delimiter=',')
    prem_vp=genfromtxt(folder+'/lookupdata/vpPREM.csv', delimiter=',')
...
def ak135vp(radius):
    ...
    ak135_vp=genfromtxt(folder+'/lookupdata/vpAK135.csv', delimiter=',')
...
def ak135radius():
    # Read PREM data from CSV files
    ak135_radius= genfromtxt(folder+'/lookupdata/radAK135.csv', delimiter=',')
...
def prempressure(radius): # pressure given radius
    ...
    # Read PREM data from CSV files
    prem_radius= genfromtxt(folder+'/lookupdata/radPREM.csv', delimiter=',')
    prem_pressure=genfromtxt(folder+'/lookupdata/presPREM.csv', delimiter=',')
