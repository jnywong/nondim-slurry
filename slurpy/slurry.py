...
#        return (outputDir,0,0,0,0,0)
    ...
    # Load previous solution to initse
  ...
  ate=2
    ...
    while state == 2:
        try:
            ...
            inputDir="results/"+folderNa + str1 + "_Lix" + str2 + "_Le" + str4 + "_St" + str3   + "_Pe" + str5 + "/"
            data_output=pd.read_csv(inputDir+"ouv",index_col=False)
       ...
       state=2
            while state == 2:
                try:
                   ...
                   inputDir="results/"+folderName+"Lip" + str1x" + str2 + "_Le" + str4 + "_St" + str3   + "_Pe" + str5 + "/"
                    data_output=pd.read_csv(inputDir+"outputs.csv",index_colFalse)
                    state = np.float(data_output['state'])
                    m=m+1
                    initOn=2
...
#    # Read csv data
    if initOn!=0:
        print('Ind with {}'.format(inputDir))
        data_profile=pd.read_csv(inputDir+"profiles.csndex_col=False)
 ...
 # %% BOUNDARY VALUE PROB
    def fun_sph(r,y,p): # use ICB or snow speed?
        ...
        F=p[0]
        speed=p[1]
        # PREM gty_prem=lp.premdensity(r*csb_radius)/density0 # dimensionless density
...
#    csb_temp = 6000
  ...
  yb[0]-csb_temp*heat_capacity*Rrho/(St*latent_heat),
                yb[0]-csb_temp*heat_capacity*Rrho/(St*latent_heat),
                yb[1]-1, # CSB x                ya[2]+speed, # ICB solid flux
                yb[2], # CSB solid flux
    ...
    if initOn!=0:
        ...
        xi0=xi0/mass_conc_O
        so0=solid_flux0/(freezing_spdensity_solidFe)
...
#                    freezing_speed_old*np.pi*csb_radius**3))
    ...
    if initOn!=0:
        ...
        sol=e_bvp(fun_sph,bc_sph,h(y),p=[initial_F,initial_speed],tol=tolerance,verbose=2,max_nodes=nmax)
    ...
    if initOn!=0 and sol.status!=0: # or sol.p[0]<0 or np.log(sol.p[0])>2):
        ...
        sol=solve_bvp(fun_sph,bc_sph,r,ic_sph(y),p=[initalnow_speed],tol=tolerance,verbose=2,max_nodes = nmax)
    ...
    if sol.status==2:
        ...
        state=0 # converge  print("Converged")
    ...
    print("ICB speed is %.2e m/s" % icbut) # icb speed
    ...
    temp_out = sol.y[0,:]*csb_heatflux*1e12/(4*density0*heat_capacity* \
                  eezing_speed*np.pi*csb_radius**2)
    xi_out= sol.y[1,:]*mass_conc_O
    j_out= sol.y[2,:]*freezing_speed*density_solidFe
    j_out[-1]=0. # BVP minimises j to be close to, bot exactly zero at the CSB
    ...
    # %% POST-PROCESS
    # Slurry density
    density_slurry,phi_out,temp_fluc,xi_fluc,phi_fluc,density_fluc=gp.slurrydensity(r_out,temp_out,xi_out,j_out, \
                                         layer_thickness,mol_conc_oxygen_bulk, \
                                         sedimentation_constant  density_jump=density_slurry[0]-density_slurry[-1]
    ...
    (Q_cmb, Qs, Qs_slurry, Qs_oc, Ql, Qg, Qg_oc, Qg_slurry, cooling_rate_out,
            cmb_temp, temp_ad) = gp.heatflux(r_out,temp_out,xi_out,j_out,phi_out, \
                             ...
                             snow_speed_out,freezineed,icb_heatflux*1e12,
                             layer_thickness,thermal_conductivity,csb_heatflux*1e12,n)

    # %% SAVE
    sp.saveprofiles(outputDir,r_out,temp_out,xi_out,j_out,phi_out,density_slurry, \
    ...
    sp.saveinputs(outputDir,n,layer_thickness,thermal_conductivity,icb_heatflux, \
           csb_heatflux, mol_coc_oxygen_bulk,mol_conc_SSi, \
    ...
    return (outputDir, r_out, temp_out, xi_out, j_out, density_slurry)
